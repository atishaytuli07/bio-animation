...
type Props = {
  s?: number;
  tone?: string;
  /** Outline colour. Ink on cream, paper on a dark ground. */
  line?: string;
};
  ...
  return (
    <svg viewBox="0 0 100 100" style={{ width: 100 * s, height: 100 * s }} aria-hidden="true">
      ...
      {[
        ...
        <circle key={i} cx={cx} cy={cy} r={1.9} fill={line} opacity={0.28} />
      ))}
      <circle cx="50" cy="50" r="42" fill="none" stroke={line} strokeWidth="4" />
      <circle cx="57" cy="44" r="16" fill={C.lavender} stroke={line} strokeWidth="3.5" />
      {/* nucleolus + highlight */}
      <circle cx="60" cy="41" r="4.5" fill={line} opacity={0.4} />
      <ellipse
        cx="38"
        cy="34"
        rx="7"
      ...
      />
      <circle cx="33" cy="64" r="5" fill={C.paper} stroke={line} strokeWidth="2.5" />
      <circle cx="67" cy="71" r="3.5" fill={C.paper} stroke={line} strokeWidth="2" />
    </svg>
  );
}
...
export function Molecule({ s = 1, tone = C.coral, line = C.ink }: Props) {
  ...
  return (
    <svg viewBox="0 0 100 100" style={{ width: 100 * s, height: 100 * s }} aria-hidden="true">
      ...
      <circle cx="50" cy="50" r="17" fill="none" stroke={line} strokeWidth="2.5" opacity={0.5} />
      <ellipse
        cx="41"
        cy="36"
        rx="6"
      ...
      {pts.map((pt, i) => {
        ...
        return <circle key={i} cx={x} cy={y} r={6} fill={C.paper} stroke={line} strokeWidth="3" />;
      })}
    </svg>
  );
}
...
export function Enzyme({ s = 1, tone = C.green, line = C.ink }: Props) {
  ...
  return (
    <svg viewBox="0 0 100 100" style={{ width: 100 * s, height: 100 * s }} aria-hidden="true">
      ...
      <circle cx="40" cy="34" r="4" fill={C.paper} stroke={line} strokeWidth="2.5" />
    </svg>
  );
}
